    ...
    while True:
        ...
        if pygame.mouse.get_pressed(3)[0] and rat.near_mouse() and hover_root_depth > 0:
            ...
            if chopping_meter > required_power[hover_root_depth]:
                ...
                if wood_obtained > 0:
                    print("WOODS OBTAINED:", wood_obtained)

                    # change this
                    display_text.append(Text(f"+{wood_obtained}", 60, screen, round(rat.x) - rat.image.get_width() // 2, round(rat.y) - rat.image.get_height()))
                total_wood += wood_obtained
                # pygame.draw.circle(screen, "red", (px,py), 10) # makes the rat looks like a clown
        else:
            chopping_meter = 0

        t_wood = Text(f"Wood: {total_wood}", 99, screen, 25, 50)
        t_wood.draw("black", 30)

        for i in display_text:
            if i.timeleft <= 0:
                display_text.remove(i)
            else:
                wood_img = pygame.transform.scale(pygame.image.load("./assets/wood_1fab5.png"), (50, 50))
                screen.blit(wood_img, (i.x - rat.image.get_width() / 4, i.y - rat.image.get_height() / 4))
                i.draw("white", 30)
